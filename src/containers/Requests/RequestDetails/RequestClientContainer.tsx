...
import { useGetUserById } from "@/src/hooks/react-query/user";
...
export const RequestClientContainer: React.FC<RequestClientContainerProps> = ({
  requestId,
}) => {
  const { data: request, isLoading: isLoadingRequest } =
    useGetRequestById(requestId);
  const { data: client, isLoading: isLoadingUser } = useGetUserById(
    request?.client_id,
  );
