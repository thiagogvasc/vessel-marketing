...
export const useGetRequestById = (id: string | undefined | null) => {
  return useQuery(["request", id], () => id ? getRequestById(id) : Promise.resolve(null), {
...
export const useUpdateRequest = () => {
  ...
  return useMutation(({ id, updates }: { id: string; updates: Partial<Request> }) => updateRequest(id, updates), {
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries("requests");
